...
####
    start = (value['위도'], value['경도'])
    sur = env[(env.위도 < value['위도'] + 0.01) & (env.위도 > value['위도'] - 0.01)
              & (env.경도 < value['경도'] + 0.013) & (env.경도 > value['경도'] - 0.013)]
    dis = []
...
####
    ...
    """.format(name=name, adr=adr, price=price, size=size, kind=kind, year=year, date=date, fig1=fig1, fig2=fig2)
